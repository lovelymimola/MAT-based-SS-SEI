...
def train(model, loss, loss_ProxyAnchor, train_dataloader, VAT_dataloader, optimizer, awl, epoch, writer, device_num):
    model.train()
    ...
    for (data_nnl, data_vat) in zip(train_dataloader,VAT_dataloader):
        ...
        result_loss_batch = awl(classifier_loss_label_batch, classifier_loss_unlabel_batch, proxy_anchor_loss_label_batch, proxy_anchor_loss_unlabel_batch, vat_loss_label_batch, vat_loss_unlabel_batch)
    ...
    print(
        'Train Epoch: {} \tClassifier_Loss (Label): {:.6f}, Classifier_Loss (Unlabel): {:.6f}, Proxy Anchor Loss (Label): {:.6f}, Proxy Anchor Loss (Unlabel): {:.6f}, VAT Loss (Label): {:.6f}, VAT_Loss (Unlabel):{:.6f}, Combined_Loss :{:.6f}, Accuracy: {}/{} ({:0f}%)\n'.format(
        epoch,
...
def train_and_test(model, loss_function, loss_ProxyAnchor, train_dataset, VAT_dataset, val_dataset, optimizer, awl, epochs, writer, save_path, device_num, batch_size, vat_batch_size):
    ...
    for epoch in range(1, epochs + 1):
        ...
        train(model, loss_function, loss_ProxyAnchor, train_dataloader, VAT_dataloader, optimizer, awl, epoch, writer, device_num)
