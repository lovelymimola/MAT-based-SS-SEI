...
def train(model, loss, loss_center, train_dataloader, VAT_dataloader, optimizer1, awl1, optimizer2, awl2, epoch, writer, device_num):
    model.train()
    ...
    for (data_nnl, data_vat) in zip(train_dataloader,VAT_dataloader):
        ...
        if epoch % 2 == 0:
            ...
            result_loss_batch, weight_pa = awl1(classifier_loss_label_batch, classifier_loss_unlabel_batch, center_loss_label_batch, center_loss_unlabel_batch)
            ...
            center_loss_label += center_loss_label_batch.item()
            center_loss_unlabel += center_loss_unlabel_batch.item()
    ...
    if epoch % 2 == 0:
        ...
        center_loss_label /= len(train_dataloader)
        center_loss_unlabel /= len(train_dataloader)
        result_loss /= len(train_dataloader)
        print(
            'Train Epoch: {} \tClassifier_Loss (Label): {:.6f}, Classifier_Loss (Unlabel): {:.6f}, Center Loss (Label): {:.6f}, Center Loss (Unlabel): {:.6f}, Combined_Loss :{:.6f}, Accuracy: {}/{} ({:0f}%)\n'.format(
                epoch,
...
def train_and_test(model, loss_function, loss_center, train_dataset, VAT_dataset, val_dataset, optimizer1, awl1, optimizer2, awl2, epochs, writer, save_path, device_num, batch_size, vat_batch_size):
    ...
    for epoch in range(1, epochs + 1):
        ...
        train(model, loss_function, loss_center, train_dataloader, VAT_dataloader, optimizer1, awl1, optimizer2, awl2, epoch, writer, device_num)
...
class Config:
    def __init__(
        ...
        n_classes: int = 10,
        ft: int = 62,
        save_path: str = 'model_weight/CNN_MAT1_n_classes_10_10label_90unlabel_rand30_autoweight100.pth',
        device_num: int = 0,
        rand_num: int = 30,
        ):
...
def main():
    conf = Config()
    writer = SummaryWriter("logs_CNN_MAT1_n_classes_10_10label_90unlabel_rand30_autoweight100")
