...
def train(model, loss, loss_center, train_dataloader, VAT_dataloader, optimizer, awl, epoch, writer, device_num):
    model.train()
    ...
    for (data_nnl, data_vat) in zip(train_dataloader,VAT_dataloader):
        ...
        if epoch < 100:
            ...
            feature_norm_label = F.normalize(feature_label, p=2, dim=1)
            center_loss_label_batch = loss_center(feature_norm_label, target)
            center_loss_unlabel_batch = 0.0 * loss_center(feature_norm_label, target)
        ...
        if epoch >= 100:
            ...
            if len(we_can_believe) >= 2:
                feature_norm_unlabel = F.normalize(feature_unlabel, p=2, dim=1)
                center_loss_unlabel_batch = loss_center(feature_norm_unlabel[we_can_believe],targets_u[we_can_believe])

            vat_loss_label_batch = vat_loss_function(data)
            vat_loss_unlabel_batch = vat_loss_function(data_vat)

        result_loss_batch = awl(classifier_loss_label_batch, classifier_loss_unlabel_batch, center_loss_label_batch, center_loss_unlabel_batch, vat_loss_label_batch, vat_loss_unlabel_batch)
        ...
        cent_loss_label += center_loss_label_batch.item()
        cent_loss_unlabel += center_loss_unlabel_batch.item()
    ...
    cent_loss_label /= len(train_dataloader)
    cent_loss_unlabel /= len(train_dataloader)
    vat_loss_label /= len(train_dataloader)
    vat_loss_unlabel /= len(train_dataloader)
    result_loss /= len(train_dataloader)
    print(
        'Train Epoch: {} \tClassifier_Loss (Label): {:.6f}, Classifier_Loss (Unlabel): {:.6f}, Center Loss (Label): {:.6f}, Center Loss (Unlabel): {:.6f},  VAT Loss (Label): {:.6f}, VAT Loss (Unlabel): {:.6f}, Combined_Loss :{:.6f}, Accuracy: {}/{} ({:0f}%)\n'.format(
            epoch,
...
def train_and_test(model, loss_function, loss_center, train_dataset, VAT_dataset, val_dataset, optimizer, awl, epochs, writer, save_path, device_num, batch_size, vat_batch_size):
    ...
    for epoch in range(1, epochs + 1):
        ...
        train(model, loss_function, loss_center, train_dataloader, VAT_dataloader, optimizer, awl, epoch, writer, device_num)
...
def TrainDataset_prepared(n_classes, rand_num):
    ...
    return X_train_labeled, X_train_unlabeled, X_val, value_Y_train_labeled, value_Y_train_unlabeled, value_Y_val
...
class Config:
    def __init__(
        ...
        n_classes: int = 10,
        ft: int = 62,
        save_path: str = 'model_weight/CNN_MAT1_n_classes_10_10label_90unlabel_rand30_NormFeature_lr0001_autoweight100.pth',
        device_num: int = 0,
        rand_num: int = 30,
        ):
...
def main():
    conf = Config()
    writer = SummaryWriter("logs_CNN_MAT1_n_classes_10_10label_90unlabel_rand30_NormFeature_lr0001_autoweight100")
    device = torch.device("cuda:"+str(conf.device_num))

    RANDOM_SEED = 300  # any random number
    set_seed(RANDOM_SEED)

    X_train_labeled, X_train_unlabeled, X_val, value_Y_train_labeled, value_Y_train_unlabeled, value_Y_val = TrainDataset_prepared(conf.n_classes, conf.rand_num)
    ...
    optim = torch.optim.Adam([
        ...
        {'params':loss_center.parameters(),'lr':conf.lr_cent},
        ])
